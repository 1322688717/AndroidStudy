...
public class WhiteWinds extends View {
    ...
    public WhiteWinds(Context context) {
        this(context,null);
    }
    public WhiteWinds(Context context, AttributeSet attributeSet) {
        this(context,attributeSet,0);
    }
    public WhiteWinds(Context context,AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initView(context, attrs);
    }

    private void initView(Context context, AttributeSet attrs) {
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.WhiteWinds);
        if (array != null) {
            mColor = array.getColor(R.styleable.WhiteWinds_WindsColor, Color.WHITE);
    ...
    static class MsgHandler extends Handler {
        private WeakReference<WhiteWinds> mView;

        MsgHandler(WhiteWinds view) {
            mView = new WeakReference<WhiteWinds>(view);
        }

        @Override
        public void handleMessage(@NonNull Message msg) {
            WhiteWinds view = mView.get();
            if (view != null) {
